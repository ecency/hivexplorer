...
import { WITNESSES_ITEMS } from '../json_items/api_witnesses_items';


const API_Witnesses = (props: any) => {
    ...
    return (
        ...
        <div className=''>
                <h1>{_t('api_witnesses.title')}</h1>
                <p>{_t('api_witnesses.description')}</p>
            </div>
            <Accordion className={currTheme === "day" ? "accordion-day" : "accordion_night"} defaultActiveKey={['0']} alwaysOpen={true}>
                {WITNESSES_ITEMS.map((item:api_item_types,i:number)=>{
                     return(
...
export default connect(pageMapStateToProps, pageMapDispatchToProps)(withPersistentScroll(API_Witnesses));