    ...
    return(
       ...
       {Object.keys(field).map((key,i:number)=>{
        ...
        return(
            <>
              {field[key].length !== 0 && 
             <Row id={`${i})}`} key={i+field[key]} className="m-0 py-2 row-border-dotted">
        ...
        <>
            <table className="authority-table">
                ...
                {Object.keys(ownerHistory[0]).map((key:any,i:number)=>{
                    ...
                    <tr key={i+key+'ownerHistory'} className="row-border" >
                        <td className="pt-2 pl-4 pr-2 pb-2">{_t(`user-info.${key}`)}</td>
                        <td  className="pt-2 px-4 pb-2">
                            {ownerHistory[0][key]}
                        </td></tr>:<tr key={i} className="row-border" >
                      <td className="pt-2 pl-4 pr-2 pb-2">{_t(`user-info.${key}`)}</td>
                      <td  className="pt-2 px-2 pb-2">
                          {AuthorityObject(ownerHistory[0][key])}
                      </td>
                  </tr>
                        }
                      </>
